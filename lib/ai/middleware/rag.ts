...
import { getChunksByFilePaths } from '@/lib/db/queries';
...
export const ragMiddleware: LanguageModelV1Middleware = {
  transformParams: async ({ params }) => {
    ...
    const chunksWithSimilarity = chunksBySelection.map((chunk: any) => ({
      ...chunk,
    ...
    console.log('chunks found →', chunksBySelection.length);
    console.log('topKChunks found →', topKChunks);
